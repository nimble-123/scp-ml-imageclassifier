...
sap.ui.define([
	...
	return Controller.extend("io.nlsltz.scp-ml-imageclassifier.controller.Main", {
		...
		fileUploadComplete: function(oControlEvent) {
			...
			oView.getModel("scp_ml_api").setProperty("/result-" + service, null);
			oView.getModel("scp_ml_api").setProperty("/resultVisible-" + service, false);
			...
			var processResult = function(oController, data, fileName, url) {
				...
				var result = oController.getView().getModel("scp_ml_api").getProperty("/result-" + service);
				if (result) {
					result.push.apply(result, data.predictions);
				} else {
					result = data.predictions;
				}
				oController.getView().getModel("scp_ml_api").setProperty("/result-" + service, result);

				// display the result table
				oController.getView().getModel("scp_ml_api").setProperty("/resultVisible-" + service, true);
			};
			...
			if (oControlEvent.getParameters().status === 200) {
				...
				oView.getModel("scp_ml_api").setProperty("/resultVisible", false);
		...
		onPressImageClassifier: function(oControlEvent) {
			...
			var service = "imageclassifier";
			var url = oView.getModel("scp_ml_api").getProperty("/url");
			var type = "POST";
			var apiKey = oView.getModel("scp_ml_api").getProperty("/APIKey");
		...
		callService: function(oController, service, url, type, mode, apiKey, formData, fnProcessResult) {
			var ajaxSuccess = function(data, status, jqXHR) {
				// get the response as JSON and process the results
				fnProcessResult(oController, data, formData.values().next().value.name);
			...
			var ajaxError = function(jqXHR, status, message) {
				oController.getView().getModel("scp_ml_api").setProperty("/resultVisible-" + service, false);
			...
			var xhrReadyStateChange = function() {
				if (this.readyState === this.DONE) {
					if (this.status === 200) {
						// get the response as JSON and process the results
						fnProcessResult(oController, JSON.parse(this.response), formData.values().next().value.name);
					} else {
						oController.getView().getModel("scp_ml_api").setProperty("/resultVisible-" + service, false);
						MessageToast.show("Error for file : " + formData.values().next().value.name + " \n status: " + this.status + "\n message: " +
							this.response);
